...
xterm  -e "cd ${path_catkin_ws} && source devel/setup.bash && rosrun rviz rviz -d $(rospack find rvizConfig)/HomeService.rviz" &
sleep 5

xterm  -e "cd ${path_catkin_ws} && source devel/setup.bash && rosrun add_markers add_markers_time " &
sleep 5
